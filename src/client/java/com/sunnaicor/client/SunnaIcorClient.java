...
public class SunnaIcorClient implements ClientModInitializer {
    ...
    private static final class SunnaIcorArmorRenderer implements ArmorRenderer {
        ...
        public void render(PoseStack poseStack,
            ...
            switch (slot) {
                case HEAD -> {
                    var model = new SunnaIcorHelmetModel(entityModels.bakeLayer(SunnaIcorHelmetModel.LAYER_LOCATION));
                    model.copyHeadPose(contextModel.head);
                    ArmorRenderer.renderPart(poseStack, bufferSource, light, stack, model, ARMOR_TEXTURE);
                }
                case CHEST -> {
                    var model = new SunnaIcorChestplateModel(entityModels.bakeLayer(SunnaIcorChestplateModel.LAYER_LOCATION));
                    model.copyBodyPose(contextModel.body);
                    ArmorRenderer.renderPart(poseStack, bufferSource, light, stack, model, ARMOR_TEXTURE);
                }
                case LEGS -> {
                    var model = new SunnaIcorLeggingsModel(entityModels.bakeLayer(SunnaIcorLeggingsModel.LAYER_LOCATION));
                    model.copyLegPose(contextModel.rightLeg, contextModel.leftLeg);
                    ArmorRenderer.renderPart(poseStack, bufferSource, light, stack, model, ARMOR_TEXTURE);
                }
                case FEET -> {
                    var model = new SunnaIcorBootsModel(entityModels.bakeLayer(SunnaIcorBootsModel.LAYER_LOCATION));
                    model.copyLegPose(contextModel.rightLeg, contextModel.leftLeg);
                    ArmorRenderer.renderPart(poseStack, bufferSource, light, stack, model, ARMOR_TEXTURE);
                }
                default -> {
